...
BASE_DIR = os.getcwd()
RESULTS_DIR = os.path.join(BASE_DIR, 'Abaqus_Results')
MAP_PATH = os.path.join(BASE_DIR, 'name_map.json')
...
print("Script started - simulation_setup_compress.py")
...
def logger(root):
    ...
    fh = logging.FileHandler(os.path.join(root, 'simulation_setup_compress.log'))
...
def create_simulation(cae_file, base_inp, sim_inp, sample_name):
    ...
    try:
        ...
        if YIELD_PLASTIC_DATA:
            mat.Plastic(table=tuple(tuple(row) for row in YIELD_PLASTIC_DATA))
            print("Plastic yield data applied: {} points".format(len(YIELD_PLASTIC_DATA)))
        if FATIGUE_DATA:
            mat.Fatigue(table=tuple(tuple(row) for row in FATIGUE_DATA))
            print("Fatigue data applied: {} points".format(len(FATIGUE_DATA)))
    ...
    try:
        ...
        L_total = y_max - y_min
        delta = -COMPRESSION_RATIO * L_total
        z_mid = 0.05
    ...
    try:
        ...
        mdl.StaticStep(name='LoadStep', previous='Initial', timePeriod=new_timePeriod,
                       ...
                       nlgeom=ON, description='Compression load with large deformation',
    ...
    try:
        ...
        cae_path = os.path.join(out_dir, safe_name + '_simulation_compress.cae')
...
def main():
    ...
    if COMPRESSION_RATIO is None:
        raise ValueError("COMPRESSION_RATIO is not set.")
    lg = logger(RESULTS_DIR)
    sim_sum = []
    folders = [os.path.join(RESULTS_DIR, d) for d in os.listdir(RESULTS_DIR)
    ...
    for folder in folders:
        ...
        sim_inp = os.path.join(folder, safe + '_simulation_compress.inp')
        ...
        sim_sum.append({'sample': os.path.basename(folder), 'status': status,
                        'simulation_inp': sim_inp if ok else None})
    ...
    summary_file = os.path.join(RESULTS_DIR, 'simulation_setup_compress_summary.json')
