...
print("Script started - simulation_setup_shear.py")
...
def logger(root):
    ...
    fh = logging.FileHandler(os.path.join(root, 'simulation_setup_shear.log'))
        ...
        try:
            ...
            try:
                ...
                mdl.DisplacementBC(name='BC-Top-Direct', createStepName='LoadStep', region=top_node_set_bc,
                                   u1=delta, u2=0.0, u3=0.0, amplitude='RampAmplitude', fixed=OFF)
                mdl.HistoryOutputRequest(name='H-Output-Bottom-RF-Direct', createStepName='LoadStep',
                                         region=bottom_node_set_bc, variables=('RF1', 'RT1'), summation=TOTAL)
                mdl.HistoryOutputRequest(name='H-Output-Top-U-Direct', createStepName='LoadStep',
                                         region=top_node_set_bc, variables=('U1',), summation=NONE)
                return True
...
def create_simulation(cae_file, base_inp, sim_inp, sample_name):
    ...
    try:
        if os.path.exists(cae_file):
            ...
            if original_model is None:
                print("No suitable model found in CAE file, falling back to INP")
                ...
                print("Model loaded from CAE successfully")
        else:
            print("CAE file not found, using INP file")
            mdl = mdb.ModelFromInputFile(name=model_name, inputFileName=base_inp)
    ...
    print("Debug: Part has {} elements and {} nodes after loading".format(len(part.elements), len(part.nodes)))
    print("Debug: Part has {} cells, {} faces, {} edges".format(len(part.cells), len(part.faces), len(part.edges)))
    try:
        if hasattr(part, 'setElementType') and len(part.cells) > 0:
            ...
            print("Element types set to C3D8H")
    except Exception as e:
    ...
    try:
        ...
        L_total = y_max - y_min
        delta = SHEAR_DISPLACEMENT_RATIO * L_total
        z_mid = 0.05
        print("Model bounds: x=[{:.4f}, {:.4f}], y=[{:.4f}, {:.4f}]".format(x_min, x_max, y_min, y_max))
        print("Total length = {:.4f}, displacement = {:.4f}".format(L_total, delta))
    except Exception as e:
    ...
    try:
        ...
        mdl.StaticStep(name='LoadStep', previous='Initial', timePeriod=new_timePeriod,
                       ...
                       nlgeom=ON, description='Shear load with large deformation',
    ...
    try:
        ...
        print("JobFromInputFile created: {}".format(final_name))
        cae_path = os.path.join(out_dir, safe_name + '_simulation_shear.cae')
        mdb.saveAs(pathName=cae_path)
        print("Simulation CAE saved: {}".format(cae_path))
        os.chdir(orig_dir)
        return True
    except Exception as e:
...
def main():
    ...
    lg = logger(RESULTS_DIR)
    sim_sum = []
    folders = [os.path.join(RESULTS_DIR, d) for d in os.listdir(RESULTS_DIR) if os.path.isdir(os.path.join(RESULTS_DIR, d))]
    ...
    for folder in folders:
        ...
        sim_inp = os.path.join(folder, safe + '_simulation_shear.inp')
    ...
    summary_file = os.path.join(RESULTS_DIR, 'simulation_setup_shear_summary.json')
