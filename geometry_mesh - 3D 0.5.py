...
import os, csv, json, logging, mesh, math
...
def remap_nodes_to_hemisphere(part, bounds):
    L_total = (bounds[3] - bounds[2]) * SCALE_XY
    R = CURV_RATIO * L_total

    nodes = part.nodes
    all_x = [n.coordinates[0] for n in nodes]
    all_y = [n.coordinates[1] for n in nodes]
    cx = (min(all_x) + max(all_x)) / 2.0
    cy = (min(all_y) + max(all_y)) / 2.0

    r_max = max(math.sqrt((n.coordinates[0] - cx) ** 2 +
                           (n.coordinates[1] - cy) ** 2) for n in nodes)
    theta_max = r_max / R

    if theta_max >= math.pi / 2.0:
        print("WARNING: theta_max={:.3f} rad ({:.1f} deg) >= 90 deg — skipping remap"
              .format(theta_max, math.degrees(theta_max)))
        return False

    print("Hemisphere remap: R={:.2f} mm, center=({:.2f},{:.2f}), "
          "r_max={:.2f}, theta_max={:.1f} deg"
          .format(R, cx, cy, r_max, math.degrees(theta_max)))

    for i in range(len(nodes)):
        x, y, z = nodes[i].coordinates
        dx = x - cx
        dy = y - cy
        r = math.sqrt(dx * dx + dy * dy)
        phi = math.atan2(dy, dx) if r > 1e-12 else 0.0
        theta = r / R

        Xn = (R + z) * math.sin(theta) * math.cos(phi)
        Yn = (R + z) * math.sin(theta) * math.sin(phi)
        Zn = (R + z) * math.cos(theta)

        part.editNode(nodes=nodes[i:i + 1],
                      coordinate1=Xn, coordinate2=Yn, coordinate3=Zn)

    print("Remap complete: {} nodes repositioned".format(len(nodes)))
    return True
...
def create_geometry_and_mesh(csvf, out_dir, lg):
    ...
    remap_ok = remap_nodes_to_hemisphere(part, bounds)
    if not remap_ok:
        print("Hemisphere remap failed or was skipped")
        return None, None, None
